...
| <a name="input_enable_snapshot_act_as"></a> [enable\_snapshot\_act\_as](#input\_enable\_snapshot\_act\_as) | Append iam.serviceAccounts.actAs to the CloudScanner operations role.<br/><br/>Required to act as the target project's default Compute Engine service account<br/>when creating snapshot/scan resources. Without it, snapshot jobs fail with<br/>"403: does not have access to service account '<project-number>-compute@developer...'".<br/><br/>NOTE: this permission is applied at the cloudscanner-operations role's binding scope —<br/>org-wide for the organization module, folder-wide for folder, per-project for multiproject.<br/>Enable deliberately after reviewing that scope. | `bool` | `false` | no |
